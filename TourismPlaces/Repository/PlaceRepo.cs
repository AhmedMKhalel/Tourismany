...
namespace TourismPlaces.Repository
{
    public class PlaceRepo : IPlaceRepo
    {
        ...
        public IEnumerable<PlacesRetrieveViewModelForUser> GetAllPlacesForUser()
        {
            var userId = httpContextAccessor.HttpContext.User.FindFirstValue(ClaimTypes.NameIdentifier);
            var places = _context.Places.Include(p => p.Government).Where(p => p.UserId == userId).ToList();
            var placesRetrieveViewModelForUserList = new List<PlacesRetrieveViewModelForUser>();
            foreach (var place in places)
            {
                var placesRetrieveViewModelForUser = new PlacesRetrieveViewModelForUser(place, userId)
                {
